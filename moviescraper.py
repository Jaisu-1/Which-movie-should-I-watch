...
"""
Ask for the right parameters and scrape those values, take as many features as possible for data analysis
...
# For every year in the interval 1980-2017
for year_url in years_url:

    # Make a get request
    #https://www.imdb.com/search/title?title_type=feature&release_date=2000-01-01,2000-12-31&num_votes=10000,&languages=en
    response = get("https://www.imdb.com/search/title?title_type=feature&release_date="+ year_url + "-01-01," +year_url + "-12-31&num_votes=10000,&languages=en&sort=user_rating,desc")
    ...
    for movie in movie_container:
        ...
        if movie.find('div', class_ = 'ratings-metascore') is not None:
            ...
            #The rank in that year
            year_ranking.append(rank)
            rank += 1
            ...
            #The Metascore
            meta_score = movie.find('span', class_ = 'metascore').text
            metascores.append(int(meta_score))
...
test_df = pd.DataFrame({'Year-Rank' : year_ranking,
                        'Movie': names,
                        'Year':years,
                        'Imdb':imdb_ratings,
